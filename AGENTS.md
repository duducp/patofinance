...
This bug was fixed for all callback prefixes in a single session: `cat_del_yes_`, `grp_del_yes_`, `edit_group_sel_`, `edit_cat_select_`, `edit_date_select_`, `edit_date_custom_`, `edit_tags_done_`, `edit_tags_clr_`, and all `rec_*` prefixes (`rec_advance_yes_` before `rec_advance_`, `rec_skip_yes_` before `rec_skip_`, `rec_edit_field_` before `rec_edit_`, `rec_edit_set_*` before `rec_edit_`).
...
| `link_codes` | `id`, `user_id` (FK→users), `auth_id`, `code`, `expires_at`, `used` | One-time 6-char codes for Telegram→Web authentication. Bot generates, dashboard validates |
...
20260618000000_remove_link_codes_direction.sql          # Removed direction column — only telegram_to_web flow
```
...
| `getRecurrences(supabase, userId)` | `RecurrenceWithJoins[]` | List active recurrences with category+group joins |
| `getRecurrenceById(supabase, userId, recId)` | `RecurrenceWithJoins \| null` | Single recurrence with joins |
| `createRecurrence(supabase, data)` | `{error?}` | Insert recurrence with all frequency fields |
| `updateRecurrence(supabase, userId, recId, updates)` | `{error?}` | Update recurrence fields (partial) |
| `archiveRecurrence(supabase, userId, recId)` | `{error?}` | Set archived=true |
| `activateRecurrence(supabase, userId, recId, newNextDate?)` | `{error?}` | Set archived=false, optionally update next_date |
| `drainNotificationQueue(supabase, userId)` | `string[]` | Fetch and delete pending error notifications for user |
...
| `handleLogin(supabase, userId, chatId)` | `/login` | Generates 6-char code, saves to `link_codes`. User enters code on dashboard to create Supabase Auth session |
...
### `handlers/recurrences.ts` -- Recurrence management

| Function | Purpose |
|----------|---------|
| `handleRecurrences(supabase, userId, chatId)` | List all active recurrences sorted by next_date, with management buttons |
| `handleShowRecurrence(supabase, userId, chatId, recId, messageId?)` | Show recurrence detail (amount, category, frequency, next_date) with advance/skip/archive/edit |
| `handleAdvanceRecurrence(supabase, userId, chatId, recId)` | Create a transaction from the recurrence with `transaction_date = next_date`, then recalculate next_date |
| `handleSkipRecurrence(supabase, userId, chatId, recId)` | Skip next occurrence: recalculate next_date without creating a transaction |
| `handleArchiveRecurrence(supabase, userId, chatId, recId)` | Archive recurrence (stop generating new occurrences) |
| `handleActivateRecurrence(supabase, userId, chatId, recId)` | Reactivate archived recurrence; if next_date is past, recalculate to today |
| `handleEditRecurrence(supabase, userId, chatId, recId)` | Show edit action buttons (amount, description, category, group, frequency, tags, start_date) |
| `handleEditRecurrenceField(supabase, userId, chatId, recId, field)` | Show field-specific edit wizard (uses text input for amount/description/start_date/freq_detail, select for category/group/frequency) |
...
| `rec_transform_` | Transform transaction into recurrence | Sends new msg |
| `rec_close` | Close recurrence detail view | Sends new msg |
| `rec_back` | Back to recurrence list | Sends new msg |
| `rec_new` | Start recurrence creation | Sends new msg |
| `rec_show_` | Show recurrence detail | Sends new msg |
| `rec_advance_` / `rec_advance_yes_` | Advance recurrence (create next occurrence) | Sends new msg |
| `rec_skip_` / `rec_skip_yes_` | Skip next occurrence | Sends new msg |
| `rec_archive_` / `rec_archive_yes_` | Archive recurrence | Sends new msg |
| `rec_activate_` / `rec_activate_yes_` | Reactivate archived recurrence | Sends new msg |
| `rec_edit_` | Show edit menu | Sends new msg |
| `rec_edit_field_` | Edit specific field (amount/description/category/group/frequency/tags/start_date) | Sends new msg |
| `rec_edit_set_cat_` | Confirm category change | Sends new msg |
| `rec_edit_set_grp_` | Confirm group change | Sends new msg |
| `rec_edit_set_freqtype_` | Select new frequency type | Sends new msg |
| `rec_edit_set_tag_` | Toggle tag in recurrence edit | Edits msg |
| `wiz_frequency_` | Select frequency type during creation (daily=advance, others→freq_detail) | Sends new msg |
| `wiz_freq_detail_` | Weekly day-of-week selection during creation | Sends new msg |
